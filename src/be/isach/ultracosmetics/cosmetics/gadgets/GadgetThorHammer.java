                    ...
                    public void run() {
                        if (i.isValid()) {
                            if (Core.isAmmoEnabled()) {
                                getPlayer().getInventory().setItem((int) SettingsManager.getConfig().get("Gadget-Slot"), ItemFactory.create(getMaterial(), getData(), "§f§l" + Core.getCustomPlayer(getPlayer()).getAmmo(getType().toString().toLowerCase()) + " " + getName(), "Â§9Gadget"));
                            } else {
